...
All POC work MUST happen inside `.workspace/stress-test/` in the current working directory. Create it at the start, clean it up at the end.
...
Do NOT just reason from memory — go verify. **Launch sub-agents in parallel** using the Agent tool. Each verification task is independent, so run them concurrently:
...
For approved POCs, **run them in parallel where independent** using sub-agents via the Agent tool. All work goes in `.workspace/stress-test/` with a subdirectory per POC (e.g., `.workspace/stress-test/crdt-compat/`, `.workspace/stress-test/ws-scale/`).

Each POC sub-agent should:
1. Create its subdirectory under `.workspace/stress-test/`
...
Finally, clean up: `rm -rf .workspace/stress-test/`

---

## Feature Context Integration

When a feature-context is active, this skill writes validation findings:

1. **After stress-test completes:** Write the validation summary (claims tested, verified, refuted, plan updates recommended) to feature-context
2. **Data written:** claim count, verification results, POC outcomes, recommended plan amendments
